...
import {add_arrow_from, add_arrow_to, delete_arrow_to, reAddLine} from "../../element/anchor/arrow_Queue";
import {pushAction} from "../actionQueue";
import {socketPush} from "../../socket/socketQueue";

export class SetArrowToNullAction extends Base_action{
...
export function createArrowToNullAction(cmd,msg){//time
    let action=new SetArrowToNullAction("arrow_to_null",cmd,msg);
    let socket={type:"arrow_to_null",action:action};
    socketPush(socket);
    // pushAction(action);
}